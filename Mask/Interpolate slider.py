...
ON PREPOLATION:
This script prepolates outlines in foreground and mask layers and attempts to diagnose any problems. If prepolation fails for matching outlines, try the Alt Interpolation slider, which uses another algorithm for selecting contour order. If this also fails, you can order contours manually and use the slider without prepolation.
...
## if compatible, interpolate:
    ...
    class InterpWithMaskWindow:
        def __init__(self, glyph):
            ...
            self.w = Window((600, 36),"Interpolate Foreground with Mask:")
