...
if len(p_selected) == 0:
    ...
    ## Find which is first point in selection:
    if len(unselected) == 0:
        p_first = selected_points[0][1]
    else:    
        if selected_points[0][1] == 0 and selected_points[-1][1] == (len(contour_points) - 1):
            p_first = unselected[-1] + 1
        else:
            p_first = selected_points[0][1]
    
    ## Subtract first point from selection:
    g[cont_num][p_first].onCurve.selected = False
