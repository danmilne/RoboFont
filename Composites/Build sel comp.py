...
If a resulting composite glyph does not already exist in the font, this script will build it and mark it yellow. The font must be sorted (or saved, closed and reopened) before the new glyphs are officially registered in the template list.

This script does not check for exsiting outlines in composites before rebuilding them. Any composites which have been adjusted manually will be rebuilt. To avoid losing decomposed / modified accent glyphs, use the alternative script "Build sel comp_skip_outlines.py" instead.
...
print "--------------------------------"
if len(build_list) < 1:
    print "Script did not yield any composites."
else:
    print "Building..."
    ...
    if len(add_list) > 0:
        print "--------------------------------"
        for gname in add_list:
            f[gname].mark = yellow
            print gname, "added to font, marked yellow."
